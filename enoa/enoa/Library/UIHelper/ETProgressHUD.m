...
#error ETProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "ETProgressHUD.h"
#import <QuartzCore/QuartzCore.h>

NSString * const ETProgressHUDDidReceiveTouchEventNotification = @"ETProgressHUDDidReceiveTouchEventNotification";
NSString * const ETProgressHUDWillDisappearNotification = @"ETProgressHUDWillDisappearNotification";
NSString * const ETProgressHUDDidDisappearNotification = @"ETProgressHUDDidDisappearNotification";
NSString * const ETProgressHUDWillAppearNotification = @"ETProgressHUDWillAppearNotification";
NSString * const ETProgressHUDDidAppearNotification = @"ETProgressHUDDidAppearNotification";

NSString * const ETProgressHUDStatusUserInfoKey = @"ETProgressHUDStatusUserInfoKey";

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
static const CGFloat ETProgressHUDRingRadius = 14;
static const CGFloat ETProgressHUDRingThickness = 1;
static const CGFloat ETProgressHUDParallaxDepthPoints = 10;
#else
static const CGFloat ETProgressHUDRingRadius = 14;
static const CGFloat ETProgressHUDRingThickness = 6;
#endif

@interface ETProgressHUD ()

@property (nonatomic, readwrite) ETProgressHUDMaskType maskType;
...
- (void)showProgress:(float)progress
            ...
            maskType:(ETProgressHUDMaskType)hudMaskType;
...
+ (ETProgressHUD*)sharedView {
    ...
    static ETProgressHUD *sharedView;
...
+ (void)show {
    [[self sharedView] showProgress:-1 status:nil maskType:ETProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:ETProgressHUDMaskTypeNone];
}
...
+ (void)showProgress:(float)progress {
    [[self sharedView] showProgress:progress status:nil maskType:ETProgressHUDMaskTypeNone];
}

+ (void)showProgress:(float)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:ETProgressHUDMaskTypeNone];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[ETProgressHUD sharedView] displayDurationForString:string];
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case ETProgressHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case ETProgressHUDMaskTypeGradient: {
...
- (NSDictionary *)notificationUserInfo
{
    return (self.stringLabel.text ? @{ETProgressHUDStatusUserInfoKey : self.stringLabel.text} : nil);
}
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent *)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:ETProgressHUDDidReceiveTouchEventNotification object:event];
}

#pragma mark - Master show/dismiss methods

- (void)showProgress:(float)progress status:(NSString*)string maskType:(ETProgressHUDMaskType)hudMaskType {
    ...
    if(self.maskType != ETProgressHUDMaskTypeNone) {
    ...
    if(self.alpha != 1) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ETProgressHUDWillAppearNotification
                                                            object:nil
                                                          userInfo:userInfo];
        ...
        [UIView animateWithDuration:0.15
                         ...
                         completion:^(BOOL finished){
                             [[NSNotificationCenter defaultCenter] postNotificationName:ETProgressHUDDidAppearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (void)dismiss {
    NSDictionary *userInfo = [self notificationUserInfo];
    [[NSNotificationCenter defaultCenter] postNotificationName:ETProgressHUDWillDisappearNotification
                                                        object:nil
                                                      userInfo:userInfo];
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0 || self.hudView.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:ETProgressHUDDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (CAShapeLayer *)ringLayer {
    if(!_ringLayer) {
        ...
        _ringLayer = [self createRingLayerWithCenter:center radius:ETProgressHUDRingRadius lineWidth:ETProgressHUDRingThickness color:self.hudRingForegroundColor];
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:ETProgressHUDRingRadius lineWidth:ETProgressHUDRingThickness color:self.hudRingBackgroundColor];
...
- (BOOL)isClear { // used for iOS 7
    return (self.maskType == ETProgressHUDMaskTypeClear || self.maskType == ETProgressHUDMaskTypeNone);
}
...
- (UIView *)hudView {
    if(!hudView) {
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
        {
            ...
            effectX.minimumRelativeValue = @(-ETProgressHUDParallaxDepthPoints);
            effectX.maximumRelativeValue = @(ETProgressHUDParallaxDepthPoints);
            
            UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath: @"center.y" type: UIInterpolatingMotionEffectTypeTiltAlongVerticalAxis];
            effectY.minimumRelativeValue = @(-ETProgressHUDParallaxDepthPoints);
            effectY.maximumRelativeValue = @(ETProgressHUDParallaxDepthPoints);
...
- (UIColor *)hudBackgroundColor {
    ...
    return [UIColor orangeColor];//[[UIColor alloc]initWithRed:245 green:89 blue:14 alpha:1];//[UIColor whiteColor];
#else
...
- (UIColor *)hudForegroundColor {
    ...
    if(_uiHudFgColor == nil) {
        _uiHudFgColor = [[UIColor alloc]initWithRed:255 green:255 blue:255 alpha:1]; //[[[self class] appearance] hudForegroundColor];
    }
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
    return [UIImage imageNamed:@"ETProgressHUD.bundle/success-black"];
#else
    return [UIImage imageNamed:@"ETProgressHUD.bundle/success.png"];
#endif
}
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
    return [UIImage imageNamed:@"ETProgressHUD.bundle/error-black"];
#else
    return [UIImage imageNamed:@"ETProgressHUD.bundle/error.png"];
#endif
}
